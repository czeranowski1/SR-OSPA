...
{
    ui->setupUi(this);
 //   MainWindow::WavReader("bells.wav", "data.txt");
}
...
void MainWindow::on_Kompresuj_clicked()
{
    QString readName = QFileDialog::getOpenFileName(this, tr("Open File"));
    QString saveName = QFileDialog::getSaveFileName(this,tr("Save File"), "",
                                                    tr("Binary file(*.bin);;All Files (*)"));
    QByteArray ba = readName.toLatin1();
    const char *fileNameChar = ba.data();
    QByteArray sa = saveName.toLatin1();
    const char *saveNameChar = sa.data();
    std::ifstream input;
    std::ofstream output;
    input.open(fileNameChar, std::ios::binary);
    output.open(saveNameChar, std::ios::binary);
...
void MainWindow::on_Dekompresuj_clicked()
{
    QString readName = QFileDialog::getOpenFileName(this, tr("Open File"), "",
                                                    tr("Binary file(*.bin);;All Files (*)"));
    QString saveName = QFileDialog::getSaveFileName(this,tr("Save File"), "",
